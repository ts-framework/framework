const hooks = new Array<HookFunction>();
const errHooks = new Array<HookFunction>();
...
function exit(exit: boolean, code?: number, err?: Error) {
	// Helper functions
	...
	// Runs a single hook
	function runHook(syncArgCount: number, err: Error | null, hook: HookFunction) {
...
// Add a hook
function add(hook: HookExitFunction) {
...
add.hookEvent = function (event: string, code?: number, filter?: (...args: any[]) => any) {
...
// Unhook signal / event
add.unhookEvent = function (event: string) {
...
add.uncaughtExceptionHandler = function (hook: HookFunction) {
	errHooks.push(hook);
...
add.unhandledRejectionHandler = function (hook: HookFunction) {
	errHooks.push(hook);
...
// Export
export const onExit = add;

type HookCallback = () => void;
type HookExitFunction = (stepTowardExit: HookCallback) => any;
type HookFunction = (
	(() => any) &
	((stepTowardExit: HookCallback) => any) &
	((error: Error, stepTowardExit?: HookCallback) => any)
);
